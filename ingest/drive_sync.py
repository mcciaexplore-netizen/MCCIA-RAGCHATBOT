...
GOOGLE_DRIVE_ROOT_FOLDER_ID recursively and grabs every application/pdf file.
...
from .config import CONFIG, require_for_drive
...
def sync_all(dest_dir: Path = None, service=None) -> list:
    """Downloads every PDF under the configured root Drive folder that isn't
    already present locally at the same size. Returns the list of local paths
    for files it downloaded (skips already-present files).
    """
    dest_dir = dest_dir or CONFIG.local_staging_dir
    service = service or build_drive_service()
    downloaded = []
    for f in walk_pdfs(service, CONFIG.google_drive_root_folder_id):
...
if __name__ == "__main__":
    paths = sync_all()
    print(f"Downloaded {len(paths)} new/updated PDFs to {CONFIG.local_staging_dir}")
